# 课程目录 Course List｜公开版

本页用于整理 Monash IT / DS / AI 相关研究生课程代码、课程名称和基础分类。

目前公开版仅展示课程目录，后续会逐步补充部分课程样例。

---

## Master of IT｜课程目录

| 课程代码 | 课程名称 | 中文名称 |
|---|---|---|
| FIT9131 | Programming foundations in Java | Java 编程基础 |
| FIT9132 | Introduction to databases | 数据库导论 |
| FIT9136 | Introduction to Python programming | Python 编程入门 |
| FIT9137 | Introduction to computer architecture and networks | 计算机体系结构与网络导论 |
| FIT5032 | Internet applications development | 互联网应用开发 |
| FIT5046 | Mobile and distributed computing systems | 移动与分布式计算系统 |
| FIT5057 | Project management | 项目管理 |
| FIT5125 | IT research and innovation methods | IT 研究与创新方法 |
| FIT5136 | Software engineering | 软件工程 |
| FIT5137 | Advanced database technology | 高级数据库技术 |
| FIT5152 | User interface design and usability | 用户界面设计与可用性 |
| FIT5171 | System validation and verification, quality and standards | 系统验证、质量与标准 |
| FIT5225 | Cloud computing and security | 云计算与安全 |
| FIT5120 | Industry experience studio project | 行业实践项目 |
| FIT5122 | Professional practice | IT 专业实践 |
| FIT5126 | Masters thesis part 1 | 硕士论文第一部分 |
| FIT5127 | Masters thesis part 2 | 硕士论文第二部分 |
| FIT5128 | Masters thesis final | 硕士论文第三部分 |

---

## Master of Data Science｜课程目录

| 课程代码 | 课程名称 | 中文名称 |
|---|---|---|
| FIT9132 | Introduction to databases | 数据库导论 |
| FIT9136 | Introduction to Python programming | Python 编程入门 |
| FIT9137 | Introduction to computer architecture and networks | 计算机体系结构与网络导论 |
| MAT9004 | Mathematical foundations for data science and AI | 数据科学与人工智能数学基础 |
| FIT5057 | Project management | 项目管理 |
| FIT5145 | Foundations of data science | 数据科学基础 |
| FIT5147 | Data exploration and visualisation | 数据探索与可视化 |
| FIT5125 | IT research and innovation methods | IT 研究与创新方法 |
| FIT5196 | Data wrangling | 数据整理 |
| FIT5197 | Statistical data modelling | 统计数据建模 |
| FIT5202 | Data processing for big data | 大数据处理 |
| FIT5149 | Applied data analysis | 应用数据分析 |
| FIT5201 | Machine learning | 机器学习 |
| FIT5212 | Data analysis for semi-structured data | 半结构化数据分析 |
| FIT5230 | Malicious AI | 恶意 AI |
| BMS5021 | Introduction to Bioinformatics | 生物信息学导论 |
| FIT5120 | Industry experience studio project | 行业实践项目 |
| FIT5122 | Professional practice | IT 专业实践 |
| FIT5126 | Masters thesis part 1 | 硕士论文第一部分 |
| FIT5127 | Masters thesis part 2 | 硕士论文第二部分 |
| FIT5128 | Masters thesis final | 硕士论文第三部分 |

---

## Master of Artificial Intelligence｜课程目录

| 课程代码 | 课程名称 | 中文名称 |
|---|---|---|
| FIT9132 | Introduction to databases | 数据库导论 |
| FIT9136 | Introduction to Python programming | Python 编程入门 |
| FIT9137 | Introduction to computer architecture and networks | 计算机体系结构与网络导论 |
| MAT9004 | Mathematical foundations for data science and AI | 数据科学与人工智能数学基础 |
| FIT5047 | Fundamentals of artificial intelligence | 人工智能基础 |
| FIT5057 | Project management | 项目管理 |
| FIT5125 | IT research and innovation methods | IT 研究与创新方法 |
| FIT5201 | Machine learning | 机器学习 |
| FIT5215 | Deep learning | 深度学习 |
| FIT5222 | Planning and automated reasoning | 规划与自动推理 |
| FIT5226 | Multi agent systems and collective behaviour | 多智能体系统与集体行为 |
| FIT5216 | Modelling discrete optimisation problems | 离散优化问题建模 |
| FIT5217 | Natural language processing | 自然语言处理 |
| FIT5221 | Intelligent image and video analysis | 智能图像与视频分析 |
| FIT5230 | Malicious AI | 恶意 AI |
| FIT5120 | Industry experience studio project | 行业实践项目 |
| FIT5122 | Professional practice | IT 专业实践 |
| FIT5126 | Masters thesis part 1 | 硕士论文第一部分 |
| FIT5127 | Masters thesis part 2 | 硕士论文第二部分 |
| FIT5128 | Masters thesis final | 硕士论文第三部分 |

---


## 公开版说明

公开版目前主要展示课程目录和部分基础信息。

完整课程分析表会包含更详细的信息，例如：

- 课程难度
- 考核方式
- 是否有 final
- 是否需要组队
- 具体作业场景
- 学生反馈
- 学姐建议
- 求职方向匹配度
- 选课组合建议

后续会逐步开放部分课程样例。
